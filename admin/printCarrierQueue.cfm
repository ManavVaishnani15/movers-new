...
<cfdocument format="pdf" pagetype="letter" orientation="portrait" scale="50"> 
    <cfoutput query="printQueue">
        <cfif printQueue.printType is "new">
            ...
            <cfdocumentsection><cfinclude template="letters/carrierLetter.cfm" /></cfdocumentsection>
            <cfdocumentsection><cfinclude template="letters/carrierNewFlyer.cfm" /></cfdocumentsection>
             <cfquery  datasource="aaalh3x_onestep">
                insert into carrier_LETTER_SENDS(send_type,sent,cust_hook,sent_date)
                values(1,1,'#clientid#','#datenow#')
            </cfquery>
             <cfquery  datasource="aaalh3x_onestep">
                insert into carrier_LETTER_SENDS(send_type,sent,cust_hook,sent_date)
                values(104,1,'#clientid#','#datenow#')
            </cfquery>

        <cfelseif printQueue.printType is "followUp">
            <cfdocumentsection><cfinclude template="letters/carrierfollowUp.cfm" /></cfdocumentsection>
            <cfdocumentsection><cfinclude template="letters/carrierNewFlyer.cfm" /></cfdocumentsection>
            <cfquery  datasource="aaalh3x_onestep">
                insert into carrier_LETTER_SENDS(send_type,sent,cust_hook,sent_date,printqueueId)
