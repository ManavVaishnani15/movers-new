...
<cfsavecontent variable="REQUEST.letter_body" >
	<cfoutput>
		<span class="fontsize12">
			Dear #trim(getinfo.first_name)# #trim(getinfo.last_name)# or Current Occupant,<br />
			We see your home at #trim(getinfo.FROM_ADDRESS)#<cfif trim(getinfo.FROM_ADDRESS2) is not "">, #trim(getinfo.FROM_ADDRESS2)#</cfif>, listed with #trim(getinfo.realty_company)#, has just received an offer on
#dateformat(trim(getinfo.contract_date), "MM/DD/YYYY")#. <i>Congratulations!</i> Are you ready to plan your move? We can help! 
		</span>
	</cfoutput>
</cfsavecontent>
