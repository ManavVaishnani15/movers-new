...
<body bgcolor="eceeee">
    <cfif listfindnocase('asdasdasdasdasd',url.type)>
        <cfquery name="check_initial" datasource="aaalh3x_onestep">
            insert into carrier_printQueue(clientid,printType,printed,memberId, cleared, hanger)
    ...
    <cfelse>
		...
		<cfif trim(url.type) eq 'followUp'>
			...
			<cfquery name="checkFollowUp" datasource="aaalh3x_onestep">
				select * from carrier_printQueue
			...
			<cfif checkFollowUp.recordCount eq 0>
				...
				<cfif isDate(getinfo.appointment_date)>
					...
					<cfquery name="insertFollowUpLetterBeforeAppointmentDate" datasource="aaalh3x_onestep">
						insert into carrier_printQueue(clientid,printType,printed,memberId, cleared, hanger,followup_date,date_time_to_order_when_printing)
						values(
					...
					<cfquery name="qInsertLabel" datasource="aaalh3x_onestep">
						INSERT INTO carrier_letter_sends(send_type,cust_hook,sent,active,user_hook,followup_date,printqueueID)
    ...
    <cflocation url="mainCarrier.cfm?un=#un#&pw=#pw#" addtoken="no" />
</body>
</html>
