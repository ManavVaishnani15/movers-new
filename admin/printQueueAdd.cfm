...
<body bgcolor="eceeee">
    <cfif listfindnocase('hanger,hangerAppt,hanger2,hangerOffer,hangerOfferUrgent,hangerMayflower,hangerApptUrgent',url.type)>
        <cfquery name="check_initial" datasource="aaalh3x_onestep">
            ...
            values(#clientid#,'#url.type#',0,#verify.id#,0, 1)
        </cfquery>
    <cfelse>
			...
			<cfif checkFollowUp.recordCount eq 0>
				...
				<cfif isDate(getinfo.appointment_date)>
					...
					<cfquery name="insertFollowUpLetterBeforeAppointmentDate" datasource="aaalh3x_onestep">
						insert into printQueue(clientid,printType,printed,memberId, cleared, hanger,followup_date,date_time_to_order_when_printing)
						values(
					...
					<cfquery name="qInsertLabel" datasource="aaalh3x_onestep">
						INSERT INTO letter_sends(send_type,cust_hook,sent,active,user_hook,followup_date,printqueueID)
    ...
    <CFIF url.Type eq 'MayflowerLetter'>
        <cfquery name="check_initial" datasource="aaalh3x_onestep">
            insert into printQueue(clientid,printType,printed,memberId, cleared, hanger)
            values(#clientid#,'MayflowerFlyer',0,#verify.id#,0, 0)
        </cfquery>
    </CFIF>

    <cflocation url="main.cfm?un=#un#&pw=#pw#" addtoken="no" />
</body>
</html>
